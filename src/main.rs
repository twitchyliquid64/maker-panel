...
/// Shader pipeline for rasterizing a render.
#[cfg(feature = "tessellate")]
struct Render<'a> {
    mvp: vek::Mat4<f32>,
    positions: &'a Vec<[f64; 3]>,
    normals: &'a Vec<[f32; 3]>,
    light_dir: vek::Vec3<f32>,
}

impl<'a> euc::Pipeline for Render<'a> {
    type Vertex = u32; // Vertex index
    type VsOut = vek::Vec3<f32>; // Normal
    type Pixel = (u8, u8, u8, u8); // RGBA
    ...
    fn vert(&self, v_index: &Self::Vertex) -> ([f32; 4], Self::VsOut) {
        ...
        let v_pos = <vek::Vec3<f32>>::from((p[0] as f32, p[1] as f32, p[2] as f32)) + vek::Vec3::new(0.0, -0.5, 0.0); // Offset to center the teapot
        (
            // Calculate vertex position in camera space
            (self.mvp * vek::Vec4::from_point(v_pos)).into_array(),
            // Find vertex normal
            self.normals[v_index].into(),
        )
    }
    ...
    fn frag(&self, norm: &Self::VsOut) -> Self::Pixel {
        let ambient = 0.4;
        let diffuse = norm.dot(self.light_dir).max(0.0) * 0.2;
        let specular = self
            .light_dir
            .reflected(vek::Vec3::from(self.mvp * vek::Vec4::from(*norm)).normalized())
            .dot(-vek::Vec3::unit_z())
            .powf(20.0);

        let light = ambient + diffuse + specular;
        use vek::Clamp;
        let mut color = (vek::Rgba::new(0.7, 0.5, 0.7, 1.0) * light).clamped(vek::Rgba::zero(), vek::Rgba::one());
        if light > 0.0 {
            color.a = 1.0;
        }
...
impl Fmt {
    fn all_formats() -> &'static [Fmt] {
        &[
            ...
            // Fmt::Render, - exclude from list so its not generated by default
    ...
    fn serialize_to(&self, panel: &Panel, w: &mut impl std::io::Write) -> Result<(), Err> {
        match self {
            ...
            Fmt::Render => {
                ...
                let mut color = euc::buffer::Buffer2d::new([W, H], (0, 0, 0, 0));
                let mut depth = euc::buffer::Buffer2d::new([W, H], 1.0);

                let mvp = vek::Mat4::perspective_fov_rh_no(1.3, W as f32, H as f32, 0.01, 100.0)
                    * vek::Mat4::translation_3d(vek::Vec3::new(0.0, 0.0, -5.5))
                    * vek::Mat4::<f32>::scaling_3d(0.8)
                    * vek::Mat4::rotation_x(0.25)
                    * vek::Mat4::rotation_y(0.25);
                ...
                Render {
                    ...
                    positions: &verts,
                    normals: &normals,
                    light_dir: vek::Vec3::new(1.0, 1.0, 1.0).normalized(),
                }
                .draw::<euc::rasterizer::Triangles<_>, _>(
                    &inds.into_iter().map(|i| i as u32).collect::<Vec<_>>(),
                    &mut color, Some(&mut depth));


                use image::write_buffer_with_format;
                use std::io::Cursor;
                let mut out = Cursor::new(Vec::with_capacity(4 * 1024));
                write_buffer_with_format(
                    &mut out,
                    &color.as_ref().into_iter().map(|p| [p.0, p.1, p.2, p.3]).flatten().collect::<Vec<_>>(),
                    W as u32,
                    H as u32,
                ...
                w.write(&out.into_inner()).map_err(|e| Err::IO(e))?;
                Ok(())
            }
            #[cfg(feature = "tessellate")]
            Fmt::Stl => {
